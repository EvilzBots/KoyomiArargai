...
async def anime(event):
    ...
    ing = await Evil.send_message(event.chat_id, f"__Searching for__ `{input_str}` __in Anilist__")
    ...
    if json:
        ...
        if image:
            try:
                ...
                await tbot.send_file(event.chat_id, namae, caption=msg, buttons=buttons, reply_to=event.id, force_document=False)
                await ing.delete()
                os.remove(namae)
            except:
                msg += f" [\u2063]({image})"
